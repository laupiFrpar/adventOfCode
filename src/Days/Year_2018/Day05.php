 ...
 */
class Day05 extends Day2018Abstract
{
...
DESCRIPTION;
    ...
    public function getResult(int $part = DayInterface::ALL_PART): array
    {
        ...
        if (DayInterface::ALL_PART === $part || DayInterface::FIRST_PART === $part) {
            $result[] = $this->getTotalUnitsRemain();
        }

        if (DayInterface::ALL_PART === $part || DayInterface::SECOND_PART === $part) {
            $result[] = $this->getFullyReacting();
        }
    ...
    private function getTotalUnitsRemain(array $polymer = []): int
    {
        if (!count($polymer)) {
            $polymer = str_split($this->getData()[0]);
        }

        $letter = reset($polymer);

        while($letter) {    
            $nextLetter = next($polymer);
            
            if ($nextLetter && (ord($letter) === (ord($nextLetter) + 32) || ord($letter) === ord($nextLetter) - 32)) {
                $key1 = key($polymer);
                prev($polymer);
                $key2 = key($polymer);
                unset($polymer[$key1]);
    ...
    private function getFullyReacting(): int
    {
        $smallTotal = strlen($this->getData()[0]);

        for ($i = 65; $i < 91; $i++) {
            $newPolymer = str_ireplace(chr($i), '', $this->getData()[0]);
            $total = $this->getTotalUnitsRemain(str_split($newPolymer));
