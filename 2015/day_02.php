...
/**
 * Convert the content of the input to an array
 *
 * Use trim to remove the eventual empty last line of the file
 */
$present = explode("\n", trim(file_get_contents(__DIR__ . '/files/input_02.txt')));

// For each line, explode the value to an array with 3 numbers
foreach ($present as $key => $dimension) {
    $present[$key] = explode('x', $dimension);
}

$totalSquare = $totalFeet = 0;

/**
 * For each present's dimension, we calculate the total square feet of wrapping paper and the total feet of ribbon
 */
foreach ($present as $dimension) {
    list($length, $width, $height) = $dimension;
    $totalSquare += (2 * $length * $width) + (2 * $width * $height) + (2 * $height * $length) + smallest($length * $width, $width * $height, $height * $length);
    $totalFeet += smallest(2 * $length + 2 * $width, 2 * $width + 2 * $height, 2 * $height + 2 * $length) + ($length * $width * $height);
}

echo sprintf('------ Day 1 ------' . "\r\n");
echo sprintf('See the instruction on http://adventofcode.com/day/2' . "\r\n\r\n");
echo sprintf('--- Part 1 --- ' . "\r\n\r\n");
echo sprintf('They must order %d square feet of wrapping paper ' . "\r\n\r\n", $totalSquare);
echo sprintf('--- Part 2 --- ' . "\r\n\r\n");
echo sprintf('They must order %d feet of ribbon' . "\r\n\r\n", $totalFeet);
